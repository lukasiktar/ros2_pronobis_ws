...
def inference(args, model, test_save_path):
    ...
    array = np.empty((len(db_test) + 2,4), dtype='U50')
    array[0,1] = "Dice"
    array[0,2] = "HD95"
    array[0,3] = "Jaccard"
    ...
    for i_batch, sampled_batch in tqdm(enumerate(testloader)):
        ...
        print(metric_i)
        logging.info('idx %d case %s mean_dice %f mean_hd95 %f mean_jc %f' % (i_batch, case_name, np.mean(metric_i, axis=0)[0], np.mean(metric_i, axis=0)[1], np.mean(metric_i, axis=0)[2]))
    ...
    logging.info('Mean testing performance: mean_dice : %f mean_hd95 : %f mean_jc : %f ' % (mean_dice, mean_hd95, mean_jc))
...
if __name__ == "__main__":
    ...
    snapshot_path = snapshot_path+'_bs'+str(args.batch_size)+"_ev01"
